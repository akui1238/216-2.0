...
#     dil_pred = cv2.dilate(skel_pred, kernel)
#     dil_gt = cv2.dilate(skel_gt, kernel)
    
#     # 计算交集和并集
#     intersection = np.sum((dil_pred & skel_gt) | (dil_gt & skel_pred))
#     union = np.sum(skel_pred) + np.sum(skel_gt)
    
#     # 避免除以零
#     cldice = (2 * intersection) / union if union != 0 else 1.0
#     return cldice
...
def test_one_epoch(test_loader,
    ...
    with torch.no_grad():
        ...
        log_info = f'test of best model, loss: {np.mean(loss_list):.4f},miou: {miou}, f1_or_dsc: {f1_or_dsc}, accuracy: {accuracy}, \
                ...
                # 新增clDice结果输出
        # clDice_mean = np.mean(clDice_list) * 100
        # clDice_std = np.std(clDice_list) * 100
