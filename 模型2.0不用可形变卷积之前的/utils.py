...
class GT_BceDiceLoss(nn.Module):
    ...
    def downsample_2x(self,image):
        """两倍下采样：每隔一个像素取一个点"""
        return image[:,::2, ::2]

    def downsample_4x(self,image):
        """四倍下采样：每隔三个像素取一个点（步长4）"""
        return image[:,::4, ::4]
    
    # gt_pre2 = F.interpolate(gt_pre2, scale_factor=2, mode ='bilinear', align_corners=True)
# target5=F.interpolate(target, size=(gt_pre5.shape[2],gt_pre5.shape[3]), mode ='bilinear', align_corners=True)
# target2=F.interpolate(target, size=(gt_pre2.shape[2],gt_pre2.shape[3]), mode ='bilinear', align_corners=True)
    ...
    def forward(self, gt_pre, out, target):
        ...
        target5=F.interpolate(target, size=(gt_pre5.shape[2],gt_pre5.shape[3]), mode ='bilinear', align_corners=True)
        target2=F.interpolate(target, size=(gt_pre2.shape[2],gt_pre2.shape[3]), mode ='bilinear', align_corners=True)
        print('target5=====',target5.shape)
        print('target2=====',target2.shape)       

        # 去掉第 0 维
        target5 = target5.squeeze(0)
        ...
        # gt_loss = self.bcedice(gt_pre5, self.downsample_4x(target)) * 0.1 +  self.bcedice(
        #     gt_pre2, self.downsample_2x(target)) * 0.4 + self.bcedice(gt_pre1, target) * 0.5
        gt_loss = self.bcedice(gt_pre5, target5) * 0.3 +  self.bcedice(
            gt_pre2, target2) * 0.4 + self.bcedice(gt_pre1, target) * 0.5
